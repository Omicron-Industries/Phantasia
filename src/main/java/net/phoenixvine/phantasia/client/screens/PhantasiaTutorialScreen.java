...
public class PhantasiaTutorialScreen extends PhantasiaScreen {
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        ...
        // Next
        if (nav[2] > 0 && isOver((int)mx,(int)my, nav[2], nav[3], 70, 18)) { advance(); return true; }
        // Prev
        if (nav[0] > 0 && isOver((int)mx,(int)my, nav[0], nav[1], 70, 18)) { retreat(); return true; }
        // Close
        int cx = this.width - 20, cy = 6;
        if (isOver((int)mx,(int)my, cx, cy, 14, 14)) { onClose(); return true; }
        // Click anywhere to rush typewriter
        if (!textDone) { typeChars = currentSlide().text.length(); textDone = true; return true; }
    ...
    public boolean keyPressed(int key, int scan, int mods) {
        ...
        if (key == 263 /* Left */) { retreat(); return true; }
    ...
    private void renderHeader(GuiGraphics g, PhantasiaTheme theme) {
        ...
        // Close button [×]
        int cx = this.width - 20, cy = 6;
        boolean hov = isOver((int)(minecraft.mouseHandler.xpos() * this.width / minecraft.getWindow().getScreenWidth()),
                (int)(minecraft.mouseHandler.ypos() * this.height / minecraft.getWindow().getScreenHeight()), cx, cy, 14, 14);
